...
pin = None
pink = None
URDF_PATH = "/opt/workspace/G1_SDK_ENV/A2D_Omnipicker/A2D.urdf"
class PinkIKSolver:
    """Professional IK Solver using Pinocchio and Pink"""
    def __init__(self, urdf_path=URDF_PATH):
        if pin is None or pink is None:
            print("⚠️ Pinocchio or Pink not installed. IK will be disabled.")
            self.enabled = False
            return

        try:
            # Load robot model
            self.model = pin.model.URDFModel.load(urdf_path)
            self.data = self.model.createData()
            self.ik = pink.IK(self.model)
            self.enabled = True
            print(f"Pink IK Solver initialized with URDF: {urdf_path}")

            # Define end-effector frames
            self.ee_frames = {
                'left': 'hand_left_base_link',
                'right': 'hand_right_base_link'
            }

            # Identify arm joints (7 DOF per arm)
            # Based on URDF: idx21_arm_l_joint1 to idx27_arm_l_joint7
            # and idx61_arm_r_joint1 to idx67_arm_r_joint7
            self.arm_joints = {
                'left': [j for j in self.model.names_joint if 'arm_l_joint' in j],
                'right': [j for j in self.model.names_joint if 'arm_r_joint' in j]
            }
            # Ensure they are sorted by index if possible or map to indices
            self.arm_joint_ids = {
                'left': [self.model.getJointId(j) for j in self.arm_joints['left']],
                'right': [self.model.getJointId(j) for j in self.arm_joints['right']]
            }

        except Exception as e:
            print(f"Failed to initialize Pink IK Solver: {e}")
            self.enabled = False

    def solve(self, side, target_pos, current_joints):
        if not self.enabled:
            return None

        try:
            # 1. Map arm joints from SDK 7-element array to Pinocchio q
            q = np.zeros(self.model.nq)
            joint_ids = self.arm_joint_ids[side]

            # URDF joint indices in q
            q_indices = [self.model.joints[j_id].index_joint_id_in_vec() for j_id in joint_ids]
            for i, q_idx in enumerate(q_indices):
                q[q_idx] = current_joints[i]

            # 2. Define target pose (Position only)
            target_pose = pin.SE3(np.eye(3), target_pos)

            # 3. Solve using Pink's task-based approach
            # We define a position task for the end-effector
            ee_frame_id = self.model.getFrameId(self.ee_frames[side])

            # Use Pink IK to solve for the configuration
            # Since we are in a non-real-time solve_ik context,
            # we can iterate until we reach the target or hit a limit.
            ik_solver = pink.IK(self.model)

            # Define the task: End-effector position
            # We only constrain position (3 DOF) and let the solver handle the rest
            # To keep it simple and robust, we use a position-only task
            task_pos = pink.PositionTask(ik_solver, ee_frame_id, target_pose.translation)

            # Solve
            q_sol = ik_solver.solve(q, [task_pos])

            # Extract the 7 arm joints back from the full q vector
            res_joints = np.zeros(7)
            for i, q_idx in enumerate(q_indices):
                res_joints[i] = q_sol[q_idx]

            return res_joints

        except Exception as e:
            print(f"Pink IK solve failed: {e}")
            return None
...
class CollisionChecker:
    ...
    def check_collision(self, left_joints, right_joints):
        ...
        body_line = (np.array([0, 0, 0.1]), np.array([0, 0, 0.9]))
